...
async function deploy(accountId?: Number | undefined) {
  const [owner, ...accounts] = await ethers.getSigners();

  const Contract = await ethers.getContractFactory("SimonAndYaMin");
  const deploymentPromise = Contract.deploy(
    accountId === undefined ? owner.address : accounts[accountId].address
  );
...
describe("SimonAndYaMin - Deploy to other", () => {
  const OWNER_ADDRESS_ID = 15;

  function owner(accounts: Awaited<ReturnType<typeof deploy>>["accounts"]) {
    return accounts[OWNER_ADDRESS_ID];
  }

  beforeEach(async () => {
    deployment = await deploy(OWNER_ADDRESS_ID);
  });
  ...
  describe("Deployment", () => {
    it("ownership", async () => {
      ...
      for (let i = 0; i < 12; ++i) {
        expect(await contract.ownerOf(i)).to.equal(owner(accounts).address);
      }
    });
  ...
  describe("Owner transfers token", () => {
    beforeEach(async () => {
      const { contract, accounts } = deployment;
      const newContract = contract.connect(owner(accounts));
    ...
    it("does not affect other token ownership", async () => {
      const { contract, accounts } = deployment;
      expect(await contract.ownerOf(0)).to.equal(owner(accounts).address);
    });
    ...
    it("contract owner can transfer other token", async () => {
      const { contract, accounts } = deployment;
      const c = contract.connect(owner(accounts));
      await c["safeTransferFrom(address,address,uint256)"](
        accounts[0],
        accounts[1],
        1
      ...
      await Promise.all(
        recepients.map((recepient, index) =>
          Promise.all([
            expect(p)
              .to.emit(c, "Transfer")
              .withArgs(owner(accounts).address, recepient, index),
            expect(p).to.emit(c, "Locked").withArgs(index),
          ])
        )
      );
